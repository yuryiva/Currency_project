...
export default class HeaderNews extends Component {
  ...
  handleTopicChoise = (event, userDate) => {
    ...
    // console.log(moment(userDate).format("YYYY-MM-DD"));
    fetch(
      `http://newsapi.org/v2/everything?q=${this.state.topicChosen}&from=${finalDate}&to=${finalDate}&sortBy=popularity&pageSize=100&apiKey=${process.env.REACT_APP_API_NEWS_KEY}`
      // 6083f7655296403dbe11b0814fa23f2f`
    )
      .then((response) => response.json())
      .then((dataFromApi) =>
